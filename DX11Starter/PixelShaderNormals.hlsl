...
float4 main(VertexToPixel input) : SV_TARGET
{
	...
	// MUST happen before lighting 
	float3 normalFromMap = textureNormal.Sample(samplerState, input.uv).rgb * 2 - 1;
	...
	// Adjust the normal from the map using the TBN matrix
	// and use that from now on for our lighting
	input.normal = normalize(mul(normalFromMap, TBN));

	// Calculate the directional lights impact on final pixel color for each passed in light
